...
import view from './tag-management.html?raw'
...
class TagManagement extends AutoBind(Page) {
  ...
  async saveTag (tagToSave) {
    try {
      ...
      if (tag.id !== undefined) {
        const oldTag = ko.utils.arrayFirst(this.allTags(), t => t.id === tag.id)
        const updatedTag = await TagsService.updateTag(tag)
        this.allTags.replace(oldTag, updatedTag.data)
      } else {
        const newTag = await TagsService.createNewTag(tag)
