define([
  ...
  class FeatureAnalysisViewEdit extends AutoBind(Clipboard(Page)) {
    ...
    async loadDesign (id) {
      if (this.data() && (this.data().id || id === 0)) return
    ...
    setType (type) {
      ...
      const prevDesign = this.data().design()

      if (type === this.featureTypes.CRITERIA_SET) {
        const newDesign = this.previousDesign[type] || [this.getEmptyCriteriaFeatureDesign()]
        this.data().design(newDesign)
      } else {
        const newDesign = this.previousDesign[type] || null
        this.data().design(newDesign)
      }
      this.data().type(type)
