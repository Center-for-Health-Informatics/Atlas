...
import { isPermittedExportSQL } from './PermissionService'
...
import './components/iranalysis/components/ir-conceptset'
import 'databindings'
import 'circe'
...
class IRAnalysisManager extends AutoBind(Page) {
  constructor (params) {
    ...
    this.analysisCohorts = ko.pureComputed(() => {
      const analysisCohorts = { targetCohorts: ko.observableArray(), outcomeCohorts: ko.observableArray() }
          ...
          if (source) {
            ...
            if (executionInfo.status === 'COMPLETE') {
              if (prevCompletedTime !== executionInfo.completedTime) {
                const resultsPromise = this.loadResultsSummary(executionInfo.id.analysisId, source, prevStatus !== 'RUNNING' && silently).then(summaryList => {
  ...
  resolveCohortId (cohortId) {
    const cohortDef = this.cohortDefs().filter(function (def) {
  ...
  async save () {
    ...
    const irName = this.selectedAnalysis().name()
  ...
  delete () {
    if (!confirm(ko.i18n('ir.deleteConfirmation', 'Delete incidence rate analysis? Warning: deletion can not be undone!')())) { return }
