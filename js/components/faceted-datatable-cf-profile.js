...
import view from './faceted-datatable-cf-profile.html?raw'
import 'databindings'
import 'colvis'
...
function facetedDatatableProfile (params) {
  ...
  self.initCompleteCallback = function () {
    const dt = $('#profile-manager-table table').DataTable()
  ...
  function facetSetup () {
    ...
    self.facets().forEach(facet => {
      ...
      facet.group.all().forEach(group => {
        const oldMember = _.find(facet.Members, { Name: group.key })
        const member = {
          Name: group.key,
  ...
  self.updateFilters = function (data, event) {
    ...
    const facet = context.$parent
    const selected = facet.Members
      .filter(d => d.Selected)
      .map(d => d.Name)
