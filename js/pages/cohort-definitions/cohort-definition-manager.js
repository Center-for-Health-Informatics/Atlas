...
import 'databindings/expressionCartoonBinding'
import 'components/checks/warnings'
import 'components/checks/warnings-badge'
import 'conceptset-modal'
import './cohort-definition-manager.css'
import 'assets/ohdsi.util'
import 'components/cohortbuilder/InclusionRule'
...
import './components/reporting/cohort-reports/cohort-reports'
...
class CohortDefinitionManager extends AutoBind(Clipboard(Page)) {
  constructor (params) {
    ...
    this.sortedConceptSets = ko.pureComputed((d) => {
      if (this.currentCohortDefinition() != null) {
        const clone = this.currentCohortDefinition().expression().ConceptSets().slice(0)
    ...
    this.subscriptions.push(
      this.currentCohortDefinition.subscribe(() => (this.tabMode() === 'export') && this.currentCohortDefinition() && this.refreshPrintFriendly())
    )
  }
  ...
  delete () {
    if (!confirm(ko.i18n('cohortDefinitions.cohortDefinitionManager.confirms.delete', 'Delete cohort definition? Warning: deletion can not be undone!')())) { return }
  ...
  async save () {
    ...
    const cohortDefinitionName = this.currentCohortDefinition().name()
  ...
  saveConceptSet () {
    ...
    const conceptSetItems = conceptSetUitls.toRepositoryConceptSetItems(this.selectedConcepts())
  ...
  onRouterParamsChanged (params) {
    ...
    if (!this.checkifDataLoaded(cohortDefinitionId, conceptSetId, sourceKey)) {
      ...
      const source = this.sharedState.sources().find(s => s.sourceId === selectedSourceId)
      if (source) {
        const selectedSourceInfo = this.cohortDefinitionSourceInfo().find(s => s.sourceKey === source.sourceKey)
    ...
    if (this.currentCohortDefinition()) {
      try {
        ...
        for (let s = 0; s < cdmSources.length; s++) {
          ...
          const cdsi = this.getCDSI(source, sourceInfo)
          results.push(cdsi)
  ...
  reload () {
    if (this.modifiedJSON.length > 0) {
      const updatedExpression = JSON.parse(this.modifiedJSON)
  ...
  getStatusMessage (info) {
    if (info.status() === 'COMPLETE' && !info.isValid()) { return !info.isCanceled() ? 'FAILED' : 'CANCELED' } else { return info.status() === 'COMPLETE' ? 'COMPLETED' : info.status() }
  }
  ...
  async generateAnalyses ({ descr, duration, analysisIdentifiers, runHeraclesHeel, periods, rollupUtilizationVisit, rollupUtilizationDrug }) {
    if (!confirm(ko.i18nformat('cohortDefinitions.cohortDefinitionManager.confirms.generateAnalyses', 'This will run <%=descr%> and may take about <%=duration%>. Are you sure?', { descr, duration })())) {
      return
    }
  ...
  async createNewSample () {
    ...
    try {
      ...
      alert((error && error.data && error.data.payload && error.data.payload.message)
        ...
        : 'Error when creating sample, please try again later')
    } finally {
