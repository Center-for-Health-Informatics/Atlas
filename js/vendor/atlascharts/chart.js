...
import { schemeCategory20, schemeCategory20c } from 'utils/D3LegacySchemes'
...
class Chart {
  ...
  getOptions (chartSpecificDefaults, customOptions) {
    const options = Object.assign({}, {
      ...
      yFormat: d3.format('s'),
      colors: d3.scaleOrdinal(schemeCategory20.concat(schemeCategory20c)),
    },
    // clone objects
  ...
  static mapMonthYearDataToSeries (data, customOptions) {
    ...
    series.values = []
    data[options.dateField].forEach((datum, i) => {
      series.values.push({
