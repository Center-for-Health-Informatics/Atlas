...
function CriteriaGroupViewModel (params) {
  ...
  self.groupCountOptions = ko.pureComputed(function () {
    ...
    for (let i = 0; i < (self.group().CriteriaList().length + self.group().Groups().length); i++) {
  ...
  self.getDistinctOptions = function (criteria) {
    const distinctOptions = [{ id: 'DOMAIN_CONCEPT', name: 'Standard Concept' }, { id: 'START_DATE', name: 'Start Date' }]
