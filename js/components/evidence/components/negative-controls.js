...
import utils from 'components/evidence/utils'
import evidenceAPI from 'services/EvidenceAPI'
import cdmResultsAPI from 'services/CDMResultsAPI'
...
class NegativeControls extends Component {
  constructor (params) {
    ...
    this.generate = (service, event) => {
      ...
      const negativeControlsJob = evidenceAPI.generateNegativeControls(service.sourceKey(), this.conceptSet()
        .id, this.conceptSet()
    ...
    this.evaluateConceptSet = () => {
      ...
      let conceptSetValid = false
      let conceptDomainId = null
      let targetDomainId = null
      const conceptSetLength = this.selectedConcepts()
        .length
        ...
        .then((generationInfo) => {
          ...
          evidenceSources.forEach((evidenceSource, i) => {
            ...
            if (gi.length > 0) {
              ...
              const execDuration = (gi[0].executionDuration / 1000) + 's'
    ...
    this.refreshRecordCounts = (obj, event) => {
      if (event.originalEvent) {
        ...
        const negativeControls = this.negativeControls()
        const conceptIdsForNegativeControls = $.map(negativeControls, function (o, n) {
          return o.conceptId
        })
    ...
    this.loadResults = service => {
      ...
      evidenceAPI.getNegativeControls(service.sourceKey(), this.conceptSet()
        ...
        .then(results => {
          ...
          const conceptIdsForNegativeControls = $.map(results, function (o, n) {
            return o.conceptId
          })
          cdmResultsAPI.getConceptRecordCount(this.currentResultSource()
            ...
            .then(rowcounts => {
              ...
              evidenceAPI.getDrugLabelExists(service.sourceKey(), conceptIdsForLabels).then(results => {
                const negativeControls = this.addDrugLabelToResults(results, this.negativeControls(), this.targetDomainId())
                this.negativeControls(negativeControls)
    ...
    this.drugLabelExistsPercentage = ko.pureComputed(() => {
      if (this.drugLabelExists().length > 0) {
        const pct = (this.drugLabelExistsIsTrue().length / this.drugLabelExists().length) * 100
    ...
    this.toggleLabelDetails = () => {
      const newDisplay = !this.drugLabelDetailsDisplay()
      this.drugLabelDetailsDisplay(newDisplay)
    }

    this.toggleLabelDisplay = ko.pureComputed(() => {
      const displayVal = this.drugLabelDetailsDisplay() ? 'hide' : 'show'
      return displayVal
    })
    ...
    this.saveNewConceptSet = () => {
      const dtItems = $('#negControlResults table')
        .DataTable()
        .data()
