...
import view from './pathway-tableview.html?raw'
import './pathway-tableview.less'
import 'databindings'
...
class PathwayTableview extends AutoBind(Component) {
  ...
  getPathwayGroupData (pathwayGroup, pathLength) {
    const groups = pathwayGroup.pathways.reduce((acc, cur) => { // reduce pathways into a list of paths with counts
  ...
  getPathwayGroupDatatable (pathwayGroup, pathLength) {
    const pathCols = Array(MAX_PATH_LENGTH)
      .fill()
      .map((v, i) => {
    ...
    const statCols = [columnValueBuilder(ko.i18n('columns.count', 'Count')(), 'personCount')]
    const data = pathwayGroup ? this.getPathwayGroupData(pathwayGroup, pathLength) : []
  ...
  getEventCohortByRankDatatable (pathwayGroup) {
    const data = this.getEventCohortsByRank(pathwayGroup)
  ...
  getEventCohortCountsDatatable (pathwayGroup) {
    const data = this.getEventCohortCounts(pathwayGroup)
  ...
  getDistinctEventCohortCounts (pathwayGroup) {
    ...
    const pathways = pathwayGroup.pathways
    const dataMap = pathways.reduce((acc, cur) => { // reduce pathways an Array of ranks containing a Map of counts by comboId
      const visited = new Map()
      for (let i = 0; i < cur.path.length; i++) {
  ...
  getDistinctEventCohortCountsDatatable (pathwayGroup) {
    const data = this.getDistinctEventCohortCounts(pathwayGroup)
    ...
    return {
      ...
      options: {
        order: [[1, 'desc']],
        columns: [
          columnValueBuilder(ko.i18n('columns.distinctEventCohorts', 'Distinct Event Cohorts'), 'eventCohorts', (v) => ko.i18nformat('pathways.manager.executions.results.tableview.exactly', 'Exactly <%=v%>', { v })(), '70%'),
          columnValueBuilder(ko.i18n('columns.count', 'Count'), 'personCount'),
          columnValueBuilder(ko.i18n('columns.pctWithPathway', '% with Pathway'), 'pathwayPercent', percentFormat),
          columnValueBuilder(ko.i18n('columns.pctOfCohort', '% of Cohort'), 'cohortPercent', percentFormat)
        ],
