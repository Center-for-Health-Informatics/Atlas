...
class Sunburst extends Chart {
  ...
  getTipOffset (event, d, arc) {
    const bbox = event.target.getBBox()
    const arcCenter = arc.centroid(d)
    const tipOffsetX = Math.abs(bbox.x - arcCenter[0]) - (bbox.width / 2)
    const tipOffsetY = Math.abs(bbox.y - arcCenter[1])
  ...
  render (data, target, width, height, chartOptions) {
    ...
    const nodes = partition(root).descendants().filter(d => (d.x1 - d.x0 > options.minRadians)).reverse() // 0.005 radians = 0.29 degrees

    if (options.split) {
      const multiNodes = nodes.reduce((result, node) => {
        const splitNodes = options.split(node)
    ...
    vis.data([data]).selectAll('pathnode')
      ...
      .on('mouseover', (event, d) => self.tip.show(Object.assign({}, d, { tipDirection: self.getTipDirection(d), tipOffset: self.getTipOffset(event, d, arc) }), event.target))
      .on('mouseout', (event, d) => self.tip.hide(d, event.target))
      .on('click', (event, d) => options.onclick && options.onclick(d))
