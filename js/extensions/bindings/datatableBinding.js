...
ko.bindingHandlers.dataTable = {
  ...
  init: function (element, valueAccessor, allBindings, viewModel, bindingContext) {
    ...
    if (binding.options) {
      ...
      const selectAlls = oColumns.filter(c => c.renderSelectAll && !!c.selectAll)
  ...
  update: function (element, valueAccessor) {
    ...
    const data = ko.utils.unwrapObservable(binding.data || binding)
    ...
    if (data && data.length > 0) { table.rows.add(data) }
