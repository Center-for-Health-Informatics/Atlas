...
class IRAnalysisResultsViewer extends AutoBind(Component) {
  constructor (params) {
    ...
    this.irCaption = ko.pureComputed(() => {
      let multiplier = this.rateMultiplier()
      if (multiplier >= 1000) { multiplier = (multiplier / 1000) + 'k' }
      return ko.i18nformat('ir.results.perYears', 'per <%=multiplier%> years', { multiplier })()
    })

    this.ipCaption = ko.pureComputed(() => {
      let multiplier = this.rateMultiplier()
      if (multiplier >= 1000) { multiplier = (multiplier / 1000) + 'k' }
      return ko.i18nformat('ir.results.perPersons', 'per <%=multiplier%> persons', { multiplier })()
    })
  ...
  downloadShinyApp (source) {
    const analysisId = source.info().executionInfo.id.analysisId
  ...
  calculateRate (cases, timeAtRisk) {
    if (timeAtRisk > 0) { return ((1.0 * cases) / (1.0 * timeAtRisk) * this.rateMultiplier()).toFixed(2) } else { return (0).toFixed(2) }
  }

  calculateProportion (cases, persons) {
    if (persons > 0) { return (((1.0 * cases) / (1.0 * persons)) * this.rateMultiplier()).toFixed(2) } else { return (0).toFixed(2) }
  }
