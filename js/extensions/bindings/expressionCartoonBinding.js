...
const getCrit = _.curry(function (feature, crit) {
  switch (feature) {
    ...
    case 'critorgroup':
      ...
      // falls through — not a group, so resolve it as a plain criteria instead
    case 'crit':
...
function durInterval (crit, cohdef, critType, addcrit) {
  ...
  if (critType === 'primary') {
    ...
    durext = durExt(addcrit, cohdef.obsExt, true, range.Op)
  }
...
function rangeInfo (range, feature) {
  ...
  switch (feature) {
    ...
    case 'min': // not used
      ...
      // falls through — otherwise 'min' behaves the same as 'lower'/'val'
    case 'lower':
    case 'val':
