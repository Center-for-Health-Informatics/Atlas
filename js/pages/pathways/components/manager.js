...
import PathwayAnalysis from '../PathwayAnalysis'
...
import './tabs/pathway-exec-wrapper'
import './tabs/pathway-results'
import './tabs/pathway-utils'
...
class PathwaysManager extends AutoBind(Page) {
  ...
  async load (id, version) {
    if (this.design() && (this.design().id === id || id === 0) && !version) return // this design is already loaded.
  ...
  async save () {
    ...
    const pathwayName = this.design().name()
