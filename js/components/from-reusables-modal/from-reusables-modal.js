...
import view from './from-reusables-modal.html?raw'
...
import CriteriaTypes from '../cohortbuilder/CriteriaTypes'
...
class FromReusablesModal extends AutoBind(Component) {
  ...
  submit () {
    ...
    const parentExpression = ko.unwrap(this.parentExpression)

    const maxCsId = parentExpression.ConceptSets().length > 0
    ...
    ko.utils.arrayForEach(this.selectedReusable().parameters(), p => {
      const conceptSet = p.csArray().find(cs => cs.id === p.csId())
      if (conceptSet) {
        const newId = maxCsId + i++
  ...
  replaceParametersWithConceptSets (criteriaList, csIdMap, isGroup) {
    ...
    ko.utils.arrayForEach(criteriaList, c => {
      const criteria = isGroup ? c.Criteria : c
