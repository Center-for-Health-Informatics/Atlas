...
class SourceManager extends AutoBind(Page) {
  constructor (params) {
    ...
    this.krbHostFQDN = ko.computed(() => {
      if (this.isImpalaDS() && this.isNonEmptyConnectionString()) {
        ...
        const strArray = str.match(/KrbHostFQDN=(.*?);/)
        if (strArray != null) {
    ...
    this.krbRealm = ko.computed(() => {
      if (this.isImpalaDS() && this.isNonEmptyConnectionString()) {
        const str = this.selectedSource().connectionString()
        const strArray = str.match(/KrbRealm=(.*?);/)
        if (strArray != null) {
          const matchedStr = strArray[0]
          return matchedStr.substring(matchedStr.search('=') + 1, matchedStr.length - 1)
        } else {
  ...
  async save () {
    ...
    try {
      ...
      alert(ko.unwrap(ko.i18nformat('configuration.viewEdit.source.alerts.save.error', 'The Source was not saved. <%= message %>', { message })))
    }
  }
