...
import template from './ConceptPickerTemplate.html?raw'
import Concept from './InputTypes/Concept'
import VocabularyProvider from 'services/VocabularyProvider'
import 'assets/knockout-jqueryui/dialog'
import './conceptpicker.less'
...
ConceptPickerViewModel.prototype.doImport = function () {
  ...
  const importConceptIds = this.importValues().split(',').map(function (e) { return +e })
  const uniqueConceptIds = []
  $.each(importConceptIds, function (i, el) {
    if ($.inArray(el, uniqueConceptIds) === -1) { uniqueConceptIds.push(el) }
  })

  console.info(uniqueConceptIds)
  if (uniqueConceptIds.length > 0) {
    const results = []
    let p = new $.Deferred().resolve()
