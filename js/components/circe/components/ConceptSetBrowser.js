...
import template from './ConceptSetBrowserTemplate.html?raw'
  ...
  function defaultRepositoryConceptSetSelected (conceptSet, source) {
    ...
    VocabularyProvider.getConceptSetExpression(conceptSet.id, source.url)
      .then((result) => {
        ...
        if (!isCancelled) {
          const newId = self.cohortConceptSets().length > 0 ? Math.max.apply(null, self.cohortConceptSets().map(d => d.id)) + 1 : 0
