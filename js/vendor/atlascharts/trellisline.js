...
class Trellisline extends Chart {
  render (dataByTrellis, target, w, h, chartOptions) {
    ...
    // options
    const defaults = {
      trellisSet: Object.keys(dataByTrellis),
      yTicks: 4,
      xFormat: d3.format('d'),
      yFormat: d3.format('d'),
    ...
    function mousemove (event) {
      const date = seriesScale.invert(d3.pointer(event, event.target)[0])
    ...
    function valueLabel (text, date) {
      ...
      text.each(function (d, idx) {
        ...
        if (v && v.date) {
          ...
          const yValue = (v.Y_PREVALENCE_1000PP === 0 || v.Y_PREVALENCE_1000PP)
      ...
      Object.keys(it).forEach(k => {
        ...
        items.forEach(item => {
          const { text, x, y, textAnchor, value } = item
    ...
    const trellisHeading = svg.append('g')
