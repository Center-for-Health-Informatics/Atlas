...
define(['module'], function (module) {
  ...
  const text = {
    ...
    parseName: function (name) {
      let modName; let ext; let temp
      let strip = false
      let index = name.lastIndexOf('.')
      const isRelative = name.indexOf('./') === 0 ||
                             name.indexOf('../') === 0
    ...
    load: function (name, req, onLoad, config) {
      ...
      const parsed = text.parseName(name)
      const nonStripName = parsed.moduleName +
                    (parsed.ext ? '.' + parsed.ext : '')
      const url = req.toUrl(nonStripName)
      const useXhr = (masterConfig.useXhr) ||
                         text.useXhr
    ...
    writeFile: function (pluginName, moduleName, req, write, config) {
      ...
      const extPart = parsed.ext ? '.' + parsed.ext : ''
      const nonStripName = parsed.moduleName + extPart
      // Use a '.js' file name so that it indicates it is a
      // script that can be loaded across domains.
      const fileName = req.toUrl(parsed.moduleName + extPart) + '.js'
  ...
  if (masterConfig.env === 'node' || (!masterConfig.env &&
    ...
    text.get = function (url, callback, errback, headers) {
      const xhr = text.createXhr(); let header
    ...
    text.get = function (url, callback) {
      let stringBuffer; let line
      const encoding = 'utf-8'
      const file = new java.io.File(url)
      const lineSeparator = java.lang.System.getProperty('line.separator')
      const input = new java.io.BufferedReader(new java.io.InputStreamReader(new java.io.FileInputStream(file), encoding))
      let content = ''
      try {
