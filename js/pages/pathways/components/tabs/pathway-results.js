...
import './pathway-tableview'
import './pathway-results.less'
...
class PathwayResults extends AutoBind(Component) {
    ...
    const pathway = ancestors.map(p => (p.data.name == 'end')
      ...
      : {
          names: eventCohorts.filter(c => (c.code & Number.parseInt(p.data.name)) > 0)
            .map(ec => ({ name: ec.name, color: colors(ec.code) })),
          count: p.value
        })
    return pathway
  }
  ...
  buildPathDetails (pathwayData, pathNode) {
    const pathway = this.getPathToNode(pathNode)
    ...
    const rows = pathway.map(rowBuilder)
    rows.forEach((r, i) => {
      if (i > 0) {
    ...
    Promise.all([
      ...
      executionResults.pathwayGroups.forEach(pg => {
        pg.pathways.forEach(pw => {
          if (pw.path.split('-').length < design.maxDepth) { pw.path = pw.path + '-end' }
        })
      })
