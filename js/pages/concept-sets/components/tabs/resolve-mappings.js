...
import view from './resolve-mappings.html?raw'
...
import 'components/conceptAddBox/preview/conceptset-expression-preview'
...
class ResolveConceptSetMappings extends AutoBind(Component) {
  ...
  async enrichStandardWithCounterpartsAndResultsWithSourceCodes (addedSourceCodes) {
    try {
      ...
      this.loadingResultConceptSetItems(true)
      const standardForNonStandardWithMappings = await this.loadRelatedStandardConceptsWithMapping(addedSourceCodes)
      const currentStandardsWithCounterparts = this.standardConceptsWithCounterparts()
      const combinedListOfStandardsWithMappings = [
      ...
      const currentResults = this.resultConceptSetItems()

      const addedConcepts = addedSourceCodes.map(sc => {
        return {
          ...sc,
          isSelected: ko.observable(false),
  ...
  async loadStandardWithCounterparts () {
    try {
      this.loadingStandardConceptsWithCounterparts(true)
      const standardForNonStandardWithMappings = await this.loadRelatedStandardConceptsWithMapping(this.initialIncludedConcepts())
  ...
  async loadRelatedStandardConceptsWithMapping (concepts) {
    ...
    const nonStandardConceptsIds = nonStandardConcepts.map(concept => concept.concept.CONCEPT_ID)
  ...
  handlePreview () {
    const itemsForPreview = this.resultConceptSetItems().map(item => {
      return {
        ...item,
        isExcluded: ko.observable(false),
        includeDescendants: ko.observable(false),
  ...
  resetToInitialState () {
    ...
    const initialConcepts = this.initialStandardConceptsWithCounterparts().map(initialConcept => {
      return {
