...
import './explore-prevalence'
import './explore-temporal-cohort'
import './characterization-results.less'
...
class CharacterizationViewEditResults extends AutoBind(Component) {
  ...
  updateData () {
    ...
    const { cohorts, analyses, domains } = this.selectedItems()
    const params = {
      cohortIds: cohorts,
        ...
        lodash.uniqBy(
          resultsList.map(r => ({
            ...
            domainId: this.design() && this.design().featureAnalyses && !r.isSummary
              ? (this.design().featureAnalyses.find(fa => fa.id === r.id) || { })['domain']
              : null,
  ...
  async exportAllCSV () {
    try {
      ...
      const { cohorts, analyses, domains } = filterUtils.getSelectedFilterValues(this.filterList())
      const params = {
        cohortIds: cohorts,
        analysisIds: analyses,
        domainIds: domains,
  ...
  async exportCSV (analysis, isComparative) {
    try {
      ...
      const filterParams = filterUtils.getSelectedFilterValues(this.filterList())
      const params = {
