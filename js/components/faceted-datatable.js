...
function facetedDatatable (params) {
  ...
  self.setDataObjectLocalStorage = (data, nameItem) => {
    ...
    const filterObj = filterObjString ? JSON.parse(filterObjString) : {}
    let newFilterObj = {}
...
currentPath?.includes('/concept/')
  ...
  self.facetDimensionHelper = function facetDimensionHelper (val) {
    const ret = val === null ? self.nullFacetLabel : val
    return ret
  }
      ...
      if (self.reference() != null) {
        ...
        if (self.oOptions && self.oOptions.Facets) {
          ...
          $.each(self.oOptions.Facets, function (i, facetConfig) {
            const isArray = facetConfig.isArray || false
            const dimension = self.data().dimension(function (d) {
