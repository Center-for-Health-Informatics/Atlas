...
class SplitBoxplot extends Boxplot {
  render (data, target, w, h, chartOptions) {
    ...
    let xAxisHeight = 0; let xAxisWidth = xAxisHeight
    ...
    const boxOffset = (y.bandwidth() / 2) - (boxHeight / 2)
    const whiskerHeight = boxHeight / 2
    const endMarkerSize = whiskerHeight / 10

    if (options.showMinMarkers) {
      if (endMarkerSize > yAxisWidth) { width -= 2 * endMarkerSize - yAxisWidth } else { width -= endMarkerSize } // subtract from width any endMarkerSize's exceess over the yAxis Width; otherwise subtract only the right side's end-marker width.
    }
    // reset axis ranges
    x.range([0, width])
    y.range([height, 0])
    ...
    boxplots.each(function (boxplotData) {
      ...
      parts.forEach(part => {
        const d = part.boxPlotData
        const boxplot = part.boxplot
        const boxScale = part.boxScale
        ...
        boxplot.append('rect')
          ...
          .on('mouseover', (event, d) => self.tip.show(Object.assign({}, d, { tipDirection: part.tipDirection, tipOffset: part.tipOffset }), event.target))
          .on('mouseout', (event, d) => self.tip.hide(d, event.target))
