...
class BarChart extends Chart {
  ...
  render (data, target, w, h, chartOptions) {
    ...
    svg.selectAll('.bar')
      ...
      .attr('title', (d) => {
        let tempTitle = `${d[label]}: ${this.formatters.commaseparated(d[value], ',')}`
        if (total > 0) {
          tempTitle += ` (${this.formatters.formatpercent(d[value] / total)})`
        } else {
          tempTitle += ` (${this.formatters.formatpercent(0)})`
        }
        return tempTitle
      })
      .style('fill', d => options.colors(d[label]))
      .on('mouseover', (event, d) => this.tip.show(d, event.target))
