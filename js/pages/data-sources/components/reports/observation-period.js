...
import view from './observation-period.html?raw'
import d3 from 'd3'
...
class ObservationPeriodReport extends Report {
  ...
  parseObservationLength (observationLength, [observationLengthStats]) {
    ...
    if (!histData.DATA.empty) {
      const observationLengthData = atlascharts.histogram.mapHistogram(histData)
      if (
  ...
  parseHistogramData (data) {
    ...
    if (data && data.length > 0) {
      const observationDataMapped = data.map((value) => ({
  ...
  mapDaysToYears (bpseries, chartFormatKey) {
    const dataMinY = d3.min(bpseries, (d) => d.min)
    const dataMaxY = d3.max(bpseries, (d) => d.max)
