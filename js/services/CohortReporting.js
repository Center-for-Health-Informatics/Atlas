...
const visualizationPacks = { // not implemented in the WebAPI results service
  careSite: {
    name: 'Care Site',
    reportKey: null,
    analyses: [1200, 1201]
...
function getCompletedAnalyses (source, cohortDefinitionId) {
  const promise = $.ajax(config.api.url + 'cohortresults/' + source.sourceKey + '/' + cohortDefinitionId + '/analyses')
  return promise
}

function getCompletedHeraclesHeelAnalyses (source, cohortDefinitionId) {
  const promise = $.ajax(config.api.url + 'cohortresults/' + source.sourceKey + '/' + cohortDefinitionId + '/heraclesheel?refresh=true')
  return promise
}
