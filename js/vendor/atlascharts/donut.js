  ...
  render (data, target, w, h, chartOptions) {
    ...
    if (data.length > 0) {
      ...
      const pie = d3.pie() // this will create arc data for us given a list of values
        .value((d) => {
          return d.value > 0 ? Math.max(d.value, total * 0.015) : 0
      ...
      arcs.append('path')
        ...
        .on('mouseover', (event, d) => this.tip.show(d.data, event.target))
        .on('mouseout', (event, d) => this.tip.hide(d.data, event.target))
