...
import view from './manual-mapping.html?raw'
...
class ManualMapping extends AutoBind(Component) {
  ...
  async loadNonStandardForCurrentStandard () {
    ...
    try {
      ...
      const relatedNonStandardConceptsIds = this.currentConcept.mapped_from
      const relatedNonStandardConcepts = this.resultConceptSetItems().filter(item => item.concept.STANDARD_CONCEPT === 'N' && relatedNonStandardConceptsIds.some(id => item.concept.CONCEPT_ID === id))
  ...
  mapAllSelectedToGivenStandardConcept () {
    ...
    const resultConceptSetItemsWithoutMappedNonStandardConcepts = this.resultConceptSetItems().filter(item => !selectedNonStandardConcepts.some(selectedConcept => selectedConcept.CONCEPT_ID === item.concept.CONCEPT_ID))
    ...
    const standardConceptsWithCounterpartsWithoutCurrentMappedConcept = this.standardConceptsWithCounterparts().filter(item => item.CONCEPT_ID !== standardConceptId)
